...
function HeroVturbPlayer() {
  ...
  useEffect(() => {
    ...
    smartplayer.setAttribute("id", `vid-${VTURB_PLAYER_ID}`);
    smartplayer.setAttribute(
      "style",
      "display:block;margin:0 auto;width:100%;"
    );
    container.appendChild(smartplayer);

    if (!document.querySelector(`script[src="${VTURB_PLAYER_SCRIPT_SRC}"]`)) {
      const script = document.createElement("script");
      script.src = VTURB_PLAYER_SCRIPT_SRC;
      script.async = true;
